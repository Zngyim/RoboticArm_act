...
from constants import DT, XML_DIR, START_ARM_POSE,START_ARM_POSE_PIPER_SINGLE
from constants import PUPPET_GRIPPER_POSITION_UNNORMALIZE_FN,PIPER_GRIPPER_POSITION_UNNORMALIZE_FN
from constants import MASTER_GRIPPER_POSITION_NORMALIZE_FN,PIPER_GRIPPER_POSITION_NORMALIZE_FN
...
def make_sim_env(task_name):
    ...
    if 'sim_transfer_cube' in task_name:
        ...
        xml_path = os.path.join(XML_DIR, f'piper_pick_n_place_cube.xml')
        physics = mujoco.Physics.from_xml_path(xml_path)
...
class SinglePiperTask(base.Task):
    ...
    def before_step(self, action, physics):
        ...
        gripper_action = PIPER_GRIPPER_POSITION_UNNORMALIZE_FN(normalized_gripper_action)
...
class PickNPlaceTask(SinglePiperTask):
    def __init__(self, random=None):
        super().__init__(random=random)
        self.max_reward = 3
    ...
    def initialize_episode(self, physics):
        ...
        physics.named.data.qpos[:8] = START_ARM_POSE_PIPER_SINGLE
        np.copyto(physics.data.ctrl, START_ARM_POSE_PIPER_SINGLE)
        physics.forward()  # 同步物理引擎
        print("running here2")
        super().initialize_episode(physics)
